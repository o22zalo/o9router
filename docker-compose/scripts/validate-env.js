...
checkRequired("APP_IMAGE", "main app docker image tag");
...
checkRequired("9ROUTER_JWT_SECRET", "9Router dashboard JWT signing secret", (v) =>
  v.length >= 16 && !/^change-me/.test(v) ? null : "must be changed to a long random value"
);
checkRequired("9ROUTER_INITIAL_PASSWORD", "9Router first-login password", (v) =>
  v.length >= 8 && v !== "change-me" ? null : "must be changed before production deploy"
);
checkRequired("9ROUTER_DATA_DIR", "9Router data directory", (v) =>
  v.startsWith("/") ? null : "must be an absolute container path"
);
checkPort("9ROUTER_PORT", false);
checkOptional("9ROUTER_HOSTNAME", "9Router bind host");
checkOptional("9ROUTER_NODE_ENV", "production|development|test", (v) =>
  ["production", "development", "test"].includes(v) ? null : "must be production|development|test"
);
for (const key of [
  "9ROUTER_ENABLE_REQUEST_LOGS",
  "9ROUTER_OBSERVABILITY_ENABLED",
  "9ROUTER_AUTH_COOKIE_SECURE",
  "9ROUTER_REQUIRE_API_KEY",
]) {
  checkOptional(key, "true|false", (v) => (isBool(v) ? null : "must be true|false"));
}
checkOptional("9ROUTER_BASE_URL", "9Router internal base URL", (v) => {
  try {
    const u = new URL(v);
...
checkOptional("9ROUTER_CLOUD_URL", "9Router cloud sync URL", (v) => {
  try {
    const u = new URL(v);
    return u.protocol === "http:" || u.protocol === "https:" ? null : "must start with http:// or https://";
  } catch {
...
checkOptional("RCLONE_CHECK_FLAGS", "rclone check flags");

if ((env.ENABLE_RCLONE || "false") === "true") {
  checkRequired("RCLONE_REMOTE_TARGET", "rclone sync destination remote:path", (v) =>
    /^[^:\s]+:.+/.test(v) ? null : "must use remote:path format"
  );
  checkRequired("9ROUTER_RCLONE_CONFIG_BASE64", "base64 encoded rclone.conf", (v) =>
    /^[A-Za-z0-9+/=]+$/.test(v) ? null : "must be base64 text from rclone.conf"
  );
}
