...
CHECK_FLAGS="${RCLONE_CHECK_FLAGS:---one-way}"
CONFIG_BASE64="${RCLONE_CONFIG_BASE64:-$(printenv 9ROUTER_RCLONE_CONFIG_BASE64 2>/dev/null || true)}"
...
if [ -n "$CONFIG_BASE64" ]; then
  ...
  echo "[ERROR] Hãy set 9ROUTER_RCLONE_CONFIG_BASE64 hoặc RCLONE_CONFIG_BASE64 trong .env." >&2
  exit 1
fi
...
while true; do
  ...
  if rclone check "$LOCAL_PATH" "$REMOTE_TARGET" \
       --config "$CONFIG_PATH" \
       --log-level "$LOG_LEVEL" \
       $CHECK_FLAGS \
       $EXTRA_FLAGS; then
    echo "[$TIMESTAMP] Local và remote không khác nhau, bỏ qua sync."
    sleep "$SYNC_INTERVAL"
    continue
  fi

  echo "[$TIMESTAMP] Phát hiện khác biệt, chạy rclone sync riêng (không mount FUSE)."
